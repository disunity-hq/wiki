...
established for all of the component projects and so the manifest has been
completely finalized -- however it is are #1 priority as of this report.
...
some of our ideas for how the format standard will work and so some features
will need to refactored or polished to align with those new ideas.

One major feature that was implement since MultiMod was that Disunity now
supports first-class preload assemblies. These assemblies are loaded before the
target app or game and can do all sorts of shenannigans you can't do at
...
store. We're currently looking for anyone who know's the related stack who can
...
on Unity mod development. As such, the ecosystems we're closest are those
